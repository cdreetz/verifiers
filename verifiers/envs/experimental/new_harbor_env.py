"""HarborEnv using centralized resource management."""
...
class NewHarborEnv(NewCliAgentEnv):
    """HarborEnv subclass using NewCliAgentEnv with resource management.

    This is an improved version of HarborEnv that uses SandboxManager for:
    - Atomic resource tracking (no sandbox leakage)
    - Better error tracking (errors associated with rollouts)
    - Lifecycle observability

    The API is designed to be compatible with HarborEnv for easy migration.
    """
    ...
    async def compute_reward(self, state: vf.State) -> float:
        """Execute Harbor tests inside the sandbox to compute reward."""
